...
export const getUserHikeWishlist = async (uid) => {
  ...
  for (let i = 0; i < wishlist.length; i += batchSize) {
    ...
    const queryTenHikeEntities = query(hikesRef, where(documentId(), "in", batch));
...
export const removeHikeFromWishlist = async (userId, hikeIdToRemove) => {
  if (!userId || !hikeIdToRemove) {
    throw new Error("The document ID for the UserProfile and the HikeEntity must be provided.");
  }
  ...
  try {
    ...
    console.log(`Removed hike ${hikeIdToRemove} from user ${userId}'s wishlist.`);
  } catch (error) {
