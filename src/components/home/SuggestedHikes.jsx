...
export default function SuggestedHikes() {
  ...
  const fetchHikes = useCallback(
    async (nextBatch = false) => {
      if (allLoaded) return;
      ...
      try {
        ...
        setHikes((prev) => {
          if (nextBatch) {
            ...
            const uniqueNewHikes = newHikes.filter(
              (hike) => !existingIds.has(hike.id)
            );
  ...
  return (
    <div className="w-full h-fit relative bg-gray-red-100 p-2">
      ...
      <div
        ...
        className="flex flex-nowrap overflow-x-auto overscroll-y-none snap-x snap-mandatory space-x-4 h-full px-8 py-4 no-scrollbar relative scroll-smooth"
